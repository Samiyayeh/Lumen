# Electrical power submeter for quality and energy monitoring using Wemos microcontroller

**Category:** [[Related System international]]

**Heru Supriyono, Azra Reza Satria Hogantara, and Aris Budiman. 2025. Electrical power submeter for quality and energy monitoring using Wemos microcontroller. *International Journal of Electrical and Computer Engineering* 15, 2 (2025), 2436–2444. DOI:https://doi.org/10.11591/ijece.v15i2.pp2436-2444**

- **Summary:** Supriyono, Hogantara, and Budiman develop an IoT electrical power submeter combining a PZEM-004T measurement sensor with a Wi-Fi-enabled microcontroller and cloud interface to record continuous voltage, current, and energy consumption logs with sub-one percent current measurement deviation.

- **Relevance:** The system verifies that deploying dedicated AC sub-metering sensor modules alongside low-cost Wi-Fi microcontrollers achieves 0.85% current measurement precision against commercial reference instruments, validating the hardware telemetry accuracy and data logging reliability targeted by the edge sub-metering units in the proposed system.

- **Source Reference(s):**
  - *Section 2 (Research Method):* Hardware integration of the PZEM-004T AC sensor module and Wemos D1 mini Wi-Fi microcontroller configured to stream telemetry to a cloud data platform.
  - *Section 3 (Results and Discussion, pp. 2439–2442):* Experimental calibration against commercial multimeters demonstrating 0.85% current measurement deviation and 1.35% voltage difference across variable electrical loads.
